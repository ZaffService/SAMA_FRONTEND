...
test.describe('State Persistence Tests', () => {
  ...
  test('Scenario A: Open course → Play video → Quit → Back → Data persists', async ({ page, context }) => {
    ...
    if (!initialLessonText) {
      ...
      logger.log(`✅ Initial lesson loaded: "${initialLessonText.substring(0, 50)}..."`)
    }
    ...
    logger.log(`📊 Current state: ${videoElements} videos, ${lessonElements} lessons`)
    ...
    logger.log(`📊 After back: ${videosAfter} videos, ${lessonsAfter} lessons, ${errorMessage} errors`)
  ...
  test('Scenario B: Logout → Back → Data cleared correctly', async ({ page, context }) => {
    ...
    logger.log(`📊 After logout back: ${authMessage} auth messages, ${stillLoggedIn} logout buttons`)
  ...
  test('Scenario C: Check for "No lessons" error message when data should exist', async ({ page }) => {
    ...
    logger.log(`📊 Messages: ${noDataMessages}, Actual lessons: ${actualLessons}`)

    // If we have lessons, we shouldn't have "no data" message
    if (actualLessons > 0 && noDataMessages > 0) {
      logger.error(`❌ FAIL: False positive error detected. Has ${actualLessons} lessons but showing "no data" message`)
