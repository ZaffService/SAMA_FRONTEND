...
import { CourseDetailsApi, CourseDetails, CourseProgress } from '@/infrastructure/api/course-details-api';
...
export function useCourseDetails({ courseId }: UseCourseDetailsProps) {
  ...
  const loadCourseData = useCallback(async () => {
    ...
    try {
      ...
      const modulesWithProgressData: ModuleWithProgress[] = details.modules.map((module: any) => {
        const lessonsWithProgress: LessonWithProgress[] = module.lessons.map((lesson: any) => ({
          ...lesson,
        ...
        const completedLessons = lessonsWithProgress.filter((l: LessonWithProgress) => l.completed).length;
        const moduleProgress = lessonsWithProgress.length > 0
