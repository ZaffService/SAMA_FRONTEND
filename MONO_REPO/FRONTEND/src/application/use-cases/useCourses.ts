...
export function useCourses(
  ...
  const fetchCourses = useCallback(async (
    ...
    try {
      ...
      console.log("🔄 fetchCourses appelé:", { page, query, perPage: perPage.current });
      ...
      if (!abortControllerRef.current.signal.aborted) {
        ...
        setCurrentPage(page);
        console.log(`✅ ${result.courses.length} cours chargés (page ${page}/${result.pages})`);
      }
    } catch (err) {
  ...
  const setPage = useCallback((page: number) => {
    ...
    setCurrentPage(page);
    console.log('🔧 setCurrentPage appelé - page devrait passer de', currentPage, 'à', page);
  }, []); // ✅ AUCUNE dépendance !
