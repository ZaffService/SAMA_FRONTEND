...
export class CourseDetailsApi {
  ...
  static async getCourseDetails(courseId: string): Promise<CourseDetails> {
    try {
      console.log(`📤 [COURSE-DETAILS-API] Récupération détails cours: ${courseId}`);
  ...
  static async getCourseProgress(courseId: string): Promise<CourseProgress> {
    try {
      console.log(`📤 [COURSE-PROGRESS-API] Récupération progression: ${courseId}`);
      ...
      const data = await response.json();
      console.log(`✅ [COURSE-PROGRESS-API] Progression récupérée: ${data.progressPercentage}%`);
  ...
  static async updateLessonProgress(
    ...
    try {
      console.log(`📤 [LESSON-PROGRESS-API] Mise à jour leçon ${lessonId}: ${progress}%`);
   ...
   */
  static async completeLesson(lessonId: string): Promise<{ success: boolean; message: string }> {
    try {
