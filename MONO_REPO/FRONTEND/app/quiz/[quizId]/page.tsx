...
export default function QuizPage() {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3006/quiz/${quizId}/questions`, {
          credentials: 'include',
        });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3006/course/quiz/${quizId}`, {
        method: 'POST',
        headers: {
      ...
      setResult({
        ...
        feedback: data.passed ? 'Félicitations ! Vous avez réussi le quiz.' : 'Pas réussi cette fois. Continuez vos efforts !',
      });
      setShowResults(true);
    } catch (error) {
        ...
        {currentQuestion && (
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="flex items-center justify-between">
              ...
              >
                {isLastQuestion ? (submitting ? 'Soumission...' : 'Terminer') : 'Suivant'}
              </button>
            </div>
          </div>
        )}
