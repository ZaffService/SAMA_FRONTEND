...
import { ArrowLeft, Loader2, Plus, GripVertical, Trash2, Edit3, X } from "lucide-react";
...
export function LessonEditor({ courseId, onBack, selectedModuleId: propSelectedModuleId }: LessonEditorProps) {
  ...
  const handleVideoSourceModeChange = (
    ...
    const lessonId = newLessons[index].tempId || `lesson-${index}`;
    if (mode === "file") {
      updateNewLesson(index, { videoSource: "file", videoUrl: undefined });
    } else {
      updateNewLesson(index, { videoSource: "youtube", videoFile: undefined });
      setUploadProgress((prev) => ({ ...prev, [lessonId]: 0 }));
    }
    setUploadErrors((prev) => ({ ...prev, [lessonId]: "" }));
  };
  ...
  const removeVideo = (index: number) => {
    const lessonId = newLessons[index].tempId || `lesson-${index}`;
    updateNewLesson(index, { videoFile: undefined, videoUrl: undefined });
    setUploadProgress((prev) => ({ ...prev, [lessonId]: 0 }));
    setUploadErrors((prev) => ({ ...prev, [lessonId]: "" }));
  };
    ...
    try {
      ...
      for (const lesson of newLessons) {
        if (
          ...
          toast.error(
            `Lien YouTube invalide pour la leçon "${lesson.title}"`,
          );
          return;
        }
      }
      ...
      const lessonsData = newLessons.map((lesson, index) => ({
        ...
        orderIndex: maxExistingOrderIndex + 1 + index,
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <LessonVideoSourcePicker
                        ...
                        uploadProgress={
                          uploadProgress[lesson.tempId || `lesson-${index}`] ||
                          0
                        }
                        onModeChange={(mode) =>
