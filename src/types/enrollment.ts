...
> = {
  ...
  [EnrollmentErrorCode.PAYMENT_TIMEOUT]: {
    message:
      "La vérification du paiement a expiré. Veuillez contacter le support.",
    action: "contact",
  },
...
export function getEnrollmentError(errorData: any): EnrollmentError {
  ...
  const code =
    ...
    EnrollmentErrorCode.UNKNOWN_ERROR;
  const message =
    errorData?.message || errorData?.error?.message || errorData?.errorMessage;

  // Validate the code
  const validCode = Object.values(EnrollmentErrorCode).includes(
